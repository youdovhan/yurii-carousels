...
const outDir   = args[1] ? path.resolve(__dirname, args[1]) : path.dirname(htmlFile);

if (!fs.existsSync(htmlFile)) { console.error('No HTML file at', htmlFile); process.exit(1); }
...
(async () => {
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    const outPath = path.join(outDir, `slide_${String(i+1).padStart(2,'0')}.jpg`);
